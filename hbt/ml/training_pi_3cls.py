...
#inputs = [f"{particle}.{input_name}" for particle in particle_names for input_name in features]
...
def change_to_tf_datasets(input_array_combined, target_array_combined, sample_weights_combined):
    ...
    x = ak_2_regular_array(input_array_combined, features, dtype=dtype)
...
def ak_2_regular_array(input_array, features, dtype):
    # flatten events, such that only a 2D array with the pions and their features remain
...
if __name__ == '__main__':
    ...
    model_name = "actf_elu_500_epochs_4_layers_128_nodes_lr_0.001_nW"
    ...
    # @tf.keras.saving.register_keras_serializable(package="my_package", name="lr_metric")
